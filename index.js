...
let newdifference = ages[ages.length - 1] - ages[0]; // Subtract the first element from the last
...
newdifference = ages[ages.length - 1] - ages[0]; // Still subtracts the first element from the last
console.log(newdifference);
...
for (let i = 0; i < ages.length; i++) {
    sum += ages[i];     //Summing all the elements
}
let averageAge = sum / ages.length;     // Calculating the average
console.log(averageAge);    //The subtraction between the first and last element is dynamic and works no matter how many elements are in the array.
...
//SOLUTION:
value = ('James','John','Jane','Judy');
let  nameLengths = (5,4,4,4);
for (let i = 0; i < value .length; i++ ) {
    nameLengths[i] = value[i].length;  // Add the length of each name to nameLengths
}
console.log(nameLengths);  // Output: [5, 4, 4, 4]

//QUESTION 6:
//Write a loop to iterate over the nameLengths array and calculate the sum of all the elements in the array.

//SOLUTION:
nameLengths = [5, 5, 5, 4]; // Example array of name lengths
let totalSum = 0;  // Variable to hold the sum

// Loop through the nameLengths array
for (let i = 0; i < nameLengths.length; i++) {
    sum += nameLengths[i];  // Add each element to the totalSum
}
...
// Example: usage
console.log(concatenateWord('Hi', 3));  // Output: 'HiHiHi'
...
console.log(getFullName('JR', 'Parcius'));  // Result: 'JR Parcius'
...
console.log(getFullName('John', 'Doe'));  // Result: 'John Doe'
console.log(getFullName('Jane', 'Doe'));     // Result:'Jane Doe'
...
console.log(compareAverages([10, 20, 30], [5, 15, 15])); // Output true 
...
// Example usage:
console.log(findMaxNumber([1, 3, 5, 7, 2]));  // Output: 7
